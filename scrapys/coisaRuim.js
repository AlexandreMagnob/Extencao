    ...
    async clickProductCards() {
      ...
      for await (const categoryIndex of [...Array(categoryDivs.length).keys()]) {
          ...
          for await (const productIndex of [...Array(productCards.length).keys()]) {
              ...
              let productContainer = document.querySelector('.modal-content.w920');
              let titleElement = productContainer.querySelector('.item-name');
              let priceElement = productContainer.querySelector('.add-to-cart-btn.btn.js-add-item');
              let imgElement = productContainer.querySelector('img');
              let descricaoElement = productContainer.querySelector('.box-text');
            ...
            let complementExpandables = document.querySelectorAll('.item-properties');
            
            for await (const complementExpandable of complementExpandables) {
              let complementElements = complementExpandable.querySelectorAll('.property');
              ...
              for await (const complementElement of complementElements) {
                let typeComplementElement = complementElement.querySelector('.description');
                ...
                let optionsElement = complementExpandable.querySelectorAll('li');
                
                for await (const optionElement of optionsElement) {
                  let optionTitleElement = optionElement.querySelector('.option-name, .name');
                  
                  let optionPriceElement = optionElement.querySelector('.option-price, .formatted-price');
                  let optionDescriptionElement = optionElement.querySelector('.option-description');
